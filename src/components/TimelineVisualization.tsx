...
const timelineEvents: TimelineEvent[] = [
  {
    ...
    description: 'Donald Trump travels on Jeffrey Epstein\'s private jet from Palm Beach to Newark. Flight logs confirm this single documented flight.',
    type: 'flight',
  ...
  },
  {
    ...
    description: 'Bill Clinton takes 26 flights on Epstein\'s "Lolita Express" to various international destinations including Africa and Asia.',
    type: 'flight',
  ...
  },
  {
    ...
    description: 'Infamous photograph taken showing Prince Andrew with Virginia Roberts (age 17) in Ghislaine Maxwell\'s London apartment.',
    type: 'document',
  ...
  },
  {
    ...
    description: 'Palm Beach Police begin investigating Epstein for sexual assault of minors. Multiple victims come forward.',
    type: 'arrest',
  ...
  },
  {
    ...
    description: 'Epstein pleads guilty to state charges, serves 13 months in county jail with work release. Controversial plea deal negotiated.',
    type: 'conviction',
  ...
  },
  {
    ...
    description: 'Virginia Roberts Giuffre files civil lawsuit against Ghislaine Maxwell, alleging sex trafficking and defamation.',
    type: 'testimony',
  ...
  },
  {
    ...
    description: 'Mark Epstein emails Jeffrey: "Ask him if Putin has the photos of Trump blowing Bubba?" - suggesting knowledge of compromising material.',
    type: 'document',
  ...
  },
  {
    ...
    description: 'FBI arrests Epstein at Teterboro Airport on sex trafficking charges. Federal indictment unsealed in New York.',
    type: 'arrest',
    people: ['Jeffrey Epstein'],
    significance: 'high',
  ...
  },
  {
    ...
    description: 'Jeffrey Epstein found dead in his Manhattan jail cell. Officially ruled suicide, but conspiracy theories persist.',
    type: 'death',
    people: ['Jeffrey Epstein'],
    significance: 'high',
  ...
  },
  {
    ...
    description: 'FBI arrests Ghislaine Maxwell in New Hampshire on charges related to Epstein\'s sex trafficking operation.',
    type: 'arrest',
  ...
  },
  {
    ...
    description: 'Ghislaine Maxwell convicted on 5 of 6 counts including sex trafficking of minors. Sentenced to 20 years in prison.',
    type: 'conviction',
    people: ['Ghislaine Maxwell'],
    significance: 'high',
...
const getTypeIcon = (type: string) => {
  switch (type) {
    case 'flight': return <TrendingUp className="h-4 w-4" />;
    case 'arrest': return <AlertTriangle className="h-4 w-4" />;
    case 'conviction': return <FileText className="h-4 w-4" />;
    case 'death': return <Clock className="h-4 w-4" />;
...
const getTypeColor = (type: string) => {
  switch (type) {
    ...
    case 'conviction': return 'bg-orange-500';
    case 'death': return 'bg-gray-600';
...
export const TimelineVisualization: React.FC<TimelineVisualizationProps> = ({ people: _people }) => {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-gradient-to-br from-green-900 to-green-700 p-6 rounded-xl">
          <div className="text-3xl font-bold text-white">
            {Math.round((Date.now() - new Date('1997-01-01').getTime()) / (1000 * 60 * 60 * 24 * 365))}
          </div>
          ...
          <div className="space-y-8">
            {sortedEvents.map((event, _index) => (
              <div key={event.id} className="relative flex items-start">
                {/* Timeline dot */}
                <div className={`absolute left-6 w-4 h-4 rounded-full ${getTypeColor(event.type)} border-2 border-gray-800 z-10`}></div>
                
                {/* Event card */}
                <div className={`ml-16 flex-1 rounded-lg border-2 p-6 ${getSignificanceColor(event.significance)}`}>
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-medium ${
                      event.significance === 'high' ? 'bg-red-600 text-white' :
                      event.significance === 'medium' ? 'bg-yellow-600 text-white' :
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {event.people.map((person) => (
                          <span key={person} className="px-3 py-1 bg-slate-700 text-slate-200 rounded-full text-sm">
                            {person}
                          </span>
                        ))}
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {event.sources.map((source) => (
                          <span key={source} className="px-2 py-1 bg-slate-600 text-slate-300 rounded text-xs font-mono">
                            {source}
                          </span>
                        ))}
                      </div>
