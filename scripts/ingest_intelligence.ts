...
export async function runIntelligencePipeline() {
  ...
  try {
    ...
    // Check if there is an interrupted run to resume
    const lastRun = (
      await db.query(
        `SELECT id FROM ingest_runs WHERE status = 'interrupted' ORDER BY created_at DESC LIMIT 1`,
      )
    ).rows[0];
    const processedDocIds = new Set<string>();

    if (lastRun) {
      console.log(`⚠️ Found interrupted run ${lastRun.id}. Resuming...`);
      // Fetch docs already processed in that run (or generally)
      // Since we don't link mentions to runs strictly 1:1 in a way that makes "resume" trivial without a join,
      // we will simple exclude documents that have ANY mentions generated by THIS pipeline version?
      // Simpler: Just exclude documents that have mentions in 'entity_mentions' linked to the interrupted run?
      // But the interrupted run ID is old. We are starting a NEW run ID.

      // Actually, "Resume" implies picking up where we left off.
      // Let's filter out documents that already have mentions.
      const processed = await db.query(`SELECT DISTINCT document_id FROM entity_mentions`);
      processed.rows.forEach((r: any) => processedDocIds.add(r.document_id));
