...
function recoverMediaTags() {
  ...
  const transcriptLinks = db.prepare(`
  ...
  const insertTag = db.prepare('INSERT OR IGNORE INTO media_item_people (media_item_id, entity_id) VALUES (?, ?)');
  ...
  const topEntities = db.prepare('SELECT id, full_name FROM entities ORDER BY mentions DESC LIMIT 100').all() as {id: number, full_name: string}[];
  const allMedia = db.prepare('SELECT id, title, file_path, description FROM media_items').all() as {id: number, title: string, file_path: string, description: string}[];
  ...
  */
  ...
  const insertStmt = db.prepare('INSERT OR IGNORE INTO media_item_people (media_item_id, entity_id) VALUES (?, ?)');
  ...
  for (const targetName of Object.values(surnameMap)) {
      if (!targetIds[targetName]) {
          const row = db.prepare('SELECT id FROM entities WHERE full_name = ?').get(targetName) as {id: number} | undefined;
  ...
  db.transaction(() => {
      for (const media of allMedia) {
          const text = ((media.title || '') + ' ' + (media.file_path || '') + ' ' + (media.description || '')).toLowerCase();
  ...
  const itemsWithoutPrimary = db.prepare('SELECT id FROM media_items WHERE entity_id IS NULL').all() as {id: number}[];
  const updatePrimary = db.prepare('UPDATE media_items SET entity_id = ? WHERE id = ?');
  
  let primaries = 0;
  db.transaction(() => {
      for (const item of itemsWithoutPrimary) {
          const tag = db.prepare('SELECT entity_id FROM media_item_people WHERE media_item_id = ? LIMIT 1').get(item.id) as {entity_id: number} | undefined;
          if (tag) {
