...
function detectEntityType(
  name: string,
  context: string,
): 'Person' | 'Organization' | 'Location' | 'Unknown' {
  const lowerName = name.toLowerCase();
  const lowerContext = context.toLowerCase();

  // Organization indicators
  if (
    /(?:Inc|LLC|Ltd|Corp|Corporation|Foundation|Trust|Group|Company|Associates|Partners|Bank|Airlines|Airways)\.?$/i.test(
      name,
    )
  ) {
    return 'Organization';
  ...
  if (
    [
      ...
      'paris',
      'london',
    ].some((loc) => lowerName.includes(loc))
  ) {
    return 'Location';
  }
  ...
  if (
    /(?:Mr\.|Mrs\.|Ms\.|Dr\.|Sir|Lord|Lady|Prince|Princess|Senator|Governor|Judge|Attorney)/i.test(
      lowerContext.slice(
        Math.max(0, lowerContext.indexOf(lowerName) - 20),
        lowerContext.indexOf(lowerName) + name.length + 20,
      ),
    )
  ) {
    return 'Person';
...
async function runExtraction() {
  ...
  const documents = db
    .prepare(
      'SELECT id, title, content FROM documents WHERE content IS NOT NULL AND LENGTH(content) > 100',
    )
    .all() as any[];
  ...
  for (const doc of documents) {
    ...
    if (processed % 100 === 0) {
      process.stdout.write(
        `\r[NER] Processed ${processed}/${documents.length} documents, ${allEntities.size} entities found`,
      );
    }
  }
  ...
  db.transaction(() => {
    for (const [docId, names] of docEntities) {
      ...
      for (let i = 0; i < Math.min(names.length, 20); i++) {
        const sourceEntity = getEntityByName.get(names[i]) as
          | { id: number; red_flag_rating: number }
          | undefined;
        if (!sourceEntity) continue;

        for (let j = i + 1; j < Math.min(names.length, 20); j++) {
          const targetEntity = getEntityByName.get(names[j]) as
            | { id: number; red_flag_rating: number }
            | undefined;
  ...
  const riskBreakdown = db
    .prepare(
      `SELECT red_flag_rating, COUNT(*) as count FROM entities GROUP BY red_flag_rating ORDER BY red_flag_rating`,
    )
    .all() as { red_flag_rating: number; count: number }[];
  console.log('\nRisk Distribution:');
