...
function simpleSentiment(text: string) {
  ...
  const score =
    pos.reduce((s, w) => s + (t.includes(w) ? 1 : 0), 0) -
    neg.reduce((s, w) => s + (t.includes(w) ? 1 : 0), 0);
...
async function reindex() {
  ...
  for (const row of ids) {
    ...
    const ttr = (() => {
      ...
      return words.length ? Math.round((uniq.size / words.length) * 1000) / 10 : 0;
    })();
    ...
    const mentionRows = db
      .prepare(
        'SELECT e.red_flag_rating as r FROM entity_mentions em JOIN entities e ON e.id = em.entity_id WHERE em.document_id = ?',
      )
      .all(id) as any[];
    const entityMentions = mentionRows.length;
    const entityDensity = wordCount
      ? Math.round((entityMentions / Math.max(1, wordCount)) * 100000) / 100
      : 0;
    const riskScore = mentionRows.length
      ? Math.round(
          (mentionRows.reduce((s, m) => s + (m.r || 0), 0) / (mentionRows.length * 5)) * 100,
        )
      : 0;
    const technical = {
      producer: meta.producer || meta.pdf_producer || undefined,
      creator: meta.creator || meta.pdf_creator || undefined,
      creationDate: meta.creationDate || doc?.dateCreated || undefined,
      modificationDate: meta.modificationDate || doc?.dateModified || undefined,
      gps: meta.gps || null,
    };
    const structural = {
      containsJavascript: meta.containsJavascript ?? false,
      fontCount: meta.fontCount ?? null,
      pdfVersion: meta.pdfVersion ?? null,
    };
    const metrics = {
      technical,
      structural,
      linguistic: { readabilityFKGL: fkgl, sentiment, typeTokenRatio: ttr },
      temporal: { businessHours, dayOfWeek },
      network: { entityDensityPer1000Words: entityDensity, riskScore },
    };
    db.prepare(
      'INSERT INTO document_forensic_metrics (document_id, metrics_json, updated_at) VALUES (?,?,CURRENT_TIMESTAMP) ON CONFLICT(document_id) DO UPDATE SET metrics_json=excluded.metrics_json, updated_at=CURRENT_TIMESTAMP',
