...
function renameSascha() {
  ...
  // 1. Update Entity
  const entityRes = db
    .prepare("UPDATE entities SET full_name = 'Sascha Barros' WHERE full_name = 'Sascha Barron'")
    .run();
  ...
  // 4. Verify
  const newEntity = db
    .prepare("SELECT id, full_name FROM entities WHERE full_name = 'Sascha Barros'")
    .get();
