...
function columnExists(table: string, column: string): boolean {
  const r = db.prepare(`PRAGMA table_info(${table})`).all() as any[];
  return r.some((c) => c.name === column);
}

if (!tableExists('merge_log')) {
  db.exec(
    `CREATE TABLE merge_log (id INTEGER PRIMARY KEY AUTOINCREMENT, src_id INTEGER NOT NULL, dst_id INTEGER NOT NULL, score REAL, timestamp TEXT DEFAULT (datetime('now')))`,
  );
}
if (!columnExists('merge_log', 'reason')) {
  db.exec(`ALTER TABLE merge_log ADD COLUMN reason TEXT`);
}
