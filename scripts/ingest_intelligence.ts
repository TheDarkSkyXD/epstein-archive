...
// TYPE HEURISTICS
const ORG_SUFFIXES = ['Inc', 'Corp', 'LLC', 'Ltd', 'Bank', 'Foundation', 'University', 'Institute', 'Association', 'Club', 'Holdings', 'Trust', 'Limited', 'Group', 'Fund'];
const LOC_TERMS = ['Street', 'Avenue', 'Boulevard', 'Drive', 'Road', 'Lane', 'Island', 'City', 'County', 'State', 'New York', 'Florida', 'Palm Beach', 'Paris', 'London', 'Airport', 'House', 'Mansion'];
...
const JUNK_TERMS = [
  'The', 'A', 'An', 'On', 'In', 'To', 'From', 'By', 'Of', 'For', 'With',
  'Mr', 'Mrs', 'Ms', 'Dr', 'Prof', 'Hon', 'Sir', 'Madam',
  'Court', 'District', 'Circuit', 'Appeal', 'Supreme',
  'Plaintiff', 'Defendant', 'Appellant', 'Appellee',
  'Judge', 'Justice', 'Attorney', 'Lawyer', 'Counsel',
  'United States', 'New York', 'Florida', 'Virgin Islands', 'Epstein', // standalone surname
  'November', 'December', 'January', 'February', // months
  'Monday', 'Tuesday', 'Wednesday', // days
  'Exhibit', 'Appendix', 'Figure', 'Table',
...
function rebuildEntityPipeline() {
  ...
  const insertEntity = db.prepare('INSERT INTO entities (full_name, entity_type, red_flag_rating) VALUES (?, ?, ?)');
  const insertMention = db.prepare('INSERT INTO entity_mentions (entity_id, document_id, mention_context, keyword) VALUES (?, ?, ?, ?)');
  ...
  const entityRows = db.prepare('SELECT id, full_name, aliases, entity_type FROM entities').all() as any[];
  ...
  while (true) {
      ...
      const markAnalyzed = db.prepare("UPDATE documents SET analyzed_at = datetime('now') WHERE id = ?");
