...
function findOrCreateAlbum(name: string): number {
  const existing = db.prepare('SELECT id FROM media_albums WHERE name = ?').get(name) as { id: number } | undefined;
...
async function ingestAllMedia(): Promise<void> {
  ...
  console.log(`📁 Collected ${images.length} images from ${new Set(images.map(i => i.album)).size} albums\n`);
  ...
  for (const image of images) {
    try {
      ...
      if (OPENAI_API_KEY && processed <= 50) { // Limit AI calls for testing
