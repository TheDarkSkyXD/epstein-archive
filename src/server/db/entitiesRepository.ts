...
export const entitiesRepository = {
    ...
    getEntityById: (id: string | number): any | null => {
        ...
        return {
            ...
            blackBookEntry: blackBookEntry ? {
                ...
                phoneNumbers: blackBookEntry.phone_numbers ? JSON.parse(blackBookEntry.phone_numbers) : [],
                addresses: blackBookEntry.addresses ? JSON.parse(blackBookEntry.addresses) : [],
                emailAddresses: blackBookEntry.email_addresses ? JSON.parse(blackBookEntry.email_addresses) : [],
    ...
    updateEntity: (id: string | number, data: any) => {
        ...
        const allowed = ['full_name', 'primary_role', 'secondary_roles', 'description', 'red_flag_rating', 'red_flag_score', 'mentions'];
    ...
    getEntitySummarySource: (entityId: number | string, topN: number = 10): any => {
         const db = getDb();
         const entity = db.prepare('SELECT id, full_name, primary_role as role FROM entities WHERE id=?').get(entityId) as any;
