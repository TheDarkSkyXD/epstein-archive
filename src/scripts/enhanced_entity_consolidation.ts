...
for (const [, group] of groupEntries) {
  ...
  for (let i = 0; i < group.length; i++) {
    ...
    for (let j = i + 1; j < group.length; j++) {
      ...
      const threshold = canonicalNormalized.length > 10 ? 0.9 : 0.85;
      ...
      if (similarity >= threshold) {
        ...
        if (mergedMisspellings <= 20) {
          console.log(
            `  Merged: "${candidate.name}" → "${canonical.name}" (${(similarity * 100).toFixed(1)}%)`,
          );
        }
      }
    }
...
const unknowns = db
  .prepare(
    `SELECT id, full_name as name FROM entities WHERE entity_type = 'Unknown' OR entity_type IS NULL`,
  )
  .all() as any[];
...
for (const entity of unknowns) {
  ...
  // Organization pattern
    ...
    /(Inc|LLC|Ltd|Corp|Foundation|Trust|Company|Group|Partners|Holdings|Capital|Bank|University|Institute)/i.test(
      name,
    )
  ) {
  ...
  // Location pattern
    ...
    /(Island|Beach|City|Street|Avenue|Road|Boulevard|County|State|Country|Hotel|Resort|Airport)/i.test(
      name,
    )
  ) {
