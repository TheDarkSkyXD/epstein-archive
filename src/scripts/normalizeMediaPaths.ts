...
const rows = db.prepare('SELECT id, path, thumbnail_path FROM media_images').all() as {
  id: number;
  path: string;
...
const updateStmt = db.prepare(
  "UPDATE media_images SET path = ?, thumbnail_path = ?, date_modified = datetime('now') WHERE id = ?",
);
