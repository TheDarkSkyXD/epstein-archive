  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      try {
        ...
        const tableCheck = db
          .prepare(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='palm_beach_properties'",
          )
          .get();
        if (!tableCheck) {
          ...
          const sample = db
            .prepare('SELECT owner_name_1, total_tax_value FROM palm_beach_properties LIMIT 5')
            .all();
...
function debug() {
  ...
  runQuery(
    'Owner Name Check',
    `SELECT owner_name_1, count(*) as c FROM palm_beach_properties GROUP BY owner_name_1 ORDER BY c DESC LIMIT 10`,
  );

  // 4. Check Known Associates Linkage
  runQuery(
    'Known Associates Linkage',
    `
