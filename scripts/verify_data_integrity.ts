...
try {
    ...
    const keyEntities = ['Jeffrey Epstein', 'Ghislaine Maxwell', 'Virginia Giuffre', 'Jean-Luc Brunel'];
    for (const name of keyEntities) {
        const entity = db.prepare('SELECT id, aliases FROM entities WHERE full_name = ?').get(name) as any;
    ...
    const relCount = db.prepare('SELECT COUNT(*) as count FROM entity_relationships').get() as { count: number };
