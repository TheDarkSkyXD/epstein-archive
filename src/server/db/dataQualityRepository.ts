...
export const dataQualityRepository = {
  ...
  getMetrics: (): DataQualityMetrics => {
    ...
    // Entity Quality
    const entityQuality = db.prepare(`
      SELECT 
        COUNT(*) as total,
    ...
    const entitiesWithMentions = db.prepare(`
      SELECT COUNT(DISTINCT entity_id) as c FROM entity_mentions
    `).get() as { c: number };
    
    const provenanceCoverage = totalDocs.c > 0 
      ? Math.round((docsWithProvenance.c / totalDocs.c) * 100 * 10) / 10 
      : 0;
  ...
  getEntityConfidence: (entityId: string | number): any => {
    ...
    const highQualitySources = db.prepare(`
