...
function tagSaschaInterviews() {
  ...
  for (const name of names) {
      const row = db.prepare('SELECT id, full_name FROM entities WHERE full_name = ?').get(name) as {id: number, full_name: string} | undefined;
      if (row) {
          entityIds.push(row.id);
          console.log(`   Resolved ${name} -> ${row.id}`);
      } else {
          console.log(`   ⚠️ Could not find exact match for ${name}, trying partial...`);
           const partial = db.prepare('SELECT id, full_name FROM entities WHERE full_name LIKE ?').get(`%${name}%`) as {id: number, full_name: string} | undefined;
           if (partial) {
  ...
  // 3. Find Media Items
  const mediaItems = db.prepare("SELECT id, title FROM media_items WHERE title LIKE 'Sascha Barron Testimony%'").all() as {id: number, title: string}[];
  console.log(`   Tagging ${mediaItems.length} media items...`);
  
  const insertLink = db.prepare('INSERT OR IGNORE INTO media_item_people (media_item_id, entity_id) VALUES (?, ?)');
  ...
  db.transaction(() => {
      for (const media of mediaItems) {
          for (const entId of entityIds) {
           ...
           db.prepare('UPDATE media_items SET entity_id = ? WHERE id = ? AND entity_id IS NULL').run(sascha!.id, media.id);
      }
  })();
