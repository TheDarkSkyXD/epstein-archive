...
const missing = db
  .prepare('SELECT COUNT(*) AS c FROM documents WHERE content LIKE ?')
  .get('%Textify%').c as number;
const types = db
  .prepare('SELECT evidence_type, COUNT(*) AS c FROM documents GROUP BY evidence_type')
  .all() as any[];
const catsSample = db
  .prepare(
    "SELECT json_extract(metadata_json,'$.categories') AS cats, COUNT(*) AS c FROM documents GROUP BY cats ORDER BY c DESC LIMIT 10",
  )
  .all() as any[];
const headersSample = db
  .prepare(
    "SELECT id, json_extract(metadata_json,'$.emailHeaders.subject') AS subj FROM documents WHERE evidence_type='email' AND subj IS NOT NULL LIMIT 5",
  )
  .all() as any[];
