...
function classify(fileName: string, filePath: string, content: string) {
  ...
  if (/plaintiff|defendant|case\s+no\.|in the\s+.*court|affidavit|deposition|complaint/i.test(first)) return 'legal_document'
...
function run() {
  const rows = db.prepare('SELECT id, file_name, file_path, content, evidence_type FROM documents').all() as any[]
  const upd = db.prepare('UPDATE documents SET evidence_type = ? WHERE id = ?')
  let updated = 0
  db.transaction(() => {
    for (const r of rows) {
      const et = classify(String(r.file_name||''), String(r.file_path||''), String(r.content||''))
      if (!r.evidence_type || r.evidence_type !== et) { upd.run(et, r.id); updated++ }
    }
  })()
