...
import { Investigation, EvidenceItem, TimelineEvent, Hypothesis as BaseHypothesis, Annotation, Investigator } from '../types/investigation';
import { EvidenceChainService } from '../services/evidenceChainService';
import { MessageSquare, Phone, Mail, Calendar, User, ArrowRight, Search, Filter, Download, AlertTriangle, X, Plus, Users, Target, FileText, BarChart3, Settings, Share2, Lock, Microscope, Network, FileSearch, Database, DollarSign } from 'lucide-react';
...
export const InvestigationWorkspace: React.FC<InvestigationWorkspaceProps> = ({
  ...
  useEffect(()=>{
    try {
      ...
      if (tab && ['overview','evidence','hypotheses','timeline','team','analytics','forensic','export'].includes(tab)) {
        setActiveTab(tab);
      } else if (did) {
  ...
  useEffect(() => {
    const fetchEvidence = async () => {
      ...
      try {
        ...
        const ev = await fetch(`/api/investigations/${selectedInvestigation.id}/evidence`).then(r=>r.json());
        setEvidenceItems(ev.map((row:any)=>({ id: String(row.id), title: row.title, description: row.description || '', type: 'document', sourceId: String(row.document_id||''), source: '', relevance: row.relevance||'high', credibility: row.credibility||'verified', extractedAt: new Date(row.extracted_at), extractedBy: row.extracted_by||'system' })));
      } catch (error) {
        ...
        for (const entityId of topEntityIds) {
          try {
            ...
            for (const rel of relationships) {
              ...
              if (nodes.some(n => n.id === targetId) && !edges.some(e => 
  ...
  const createInvestigation = async () => {
    ...
    try {
      const resp = await fetch('/api/investigations', {
        ...
        body: JSON.stringify({ title: newInvestigation.title, description: newInvestigation.description, ownerId: currentUser.id, scope: newInvestigation.hypothesis })
      });
      const inv = await resp.json();
        ...
        <div className="p-6 overflow-y-auto flex-1">
          {investigations.length === 0 ? (
            <div className="text-center py-12 border border-slate-700 rounded-xl bg-slate-800/30">
              ...
              <p className="text-slate-400 mt-2">Create your first investigation to start building evidence.</p>
              <button onClick={() => setShowNewInvestigationModal(true)} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm">New Investigation</button>
            </div>
          ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-3">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(investigation.status)}`}>{investigation.status}</span>
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getPriorityColor(investigation.priority)}`}>{investigation.priority} priority</span>
                      </div>
                    </div>
              ...
              <div>
                  ...
                  <div className="liquid-glass-card rounded-xl p-6">
                    ...
                    {selectedNetworkNode ? (
                      <div className="space-y-4">
                        <div className="flex items-start justify-between">
                          <div>
                            <h5 className="text-base font-medium text-white">{selectedNetworkNode.label}</h5>
                            <p className="text-sm text-slate-400 capitalize">{selectedNetworkNode.type}</p>
                          </div>
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between text-sm">
                            ...
                            <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                              selectedNetworkNode.metadata.riskLevel === 'critical' ? 'bg-red-900 text-red-200' :
                              selectedNetworkNode.metadata.riskLevel === 'high' ? 'bg-orange-900 text-orange-200' :
                              selectedNetworkNode.metadata.riskLevel === 'medium' ? 'bg-yellow-900 text-yellow-200' :
                          ...
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="text-white">{selectedNetworkNode.metadata.mentions || 0}</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span className="text-slate-400">Category</span>
                            <span className="text-white">{selectedNetworkNode.metadata.category || 'Uncategorized'}</span>
                          </div>
                        </div>

                        {selectedNetworkNode.metadata.documents && selectedNetworkNode.metadata.documents.length > 0 && (
                          <div>
                            <h6 className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-2">Related Documents</h6>
                            <div className="space-y-1">
                              {selectedNetworkNode.metadata.documents.map((doc, idx) => (
                                <div key={idx} className="flex items-center gap-2 text-sm text-blue-400 hover:text-blue-300 cursor-pointer">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className="liquid-glass-card p-4 rounded-xl">
                    ...
                    <p className="text-3xl font-bold text-white mt-2">{networkNodes.filter(n => n.type === 'person').length}</p>
                  </div>
                  <div className="liquid-glass-card p-4 rounded-xl">
                    <h4 className="text-sm font-medium text-slate-400 uppercase tracking-wider">Organizations</h4>
                    <p className="text-3xl font-bold text-white mt-2">{networkNodes.filter(n => n.type === 'organization').length}</p>
                  </div>
                  <div className="liquid-glass-card p-4 rounded-xl">
                    <h4 className="text-sm font-medium text-slate-400 uppercase tracking-wider">Key Locations</h4>
                    <p className="text-3xl font-bold text-white mt-2">{networkNodes.filter(n => n.type === 'location').length}</p>
                  </div>
                  <div className="liquid-glass-card p-4 rounded-xl">
                ...
                <div className="mt-6 liquid-glass-panel p-4 rounded-xl border border-white/5">
                  <h4 className="text-sm font-medium text-slate-400 uppercase tracking-wider mb-3">Connection Types</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="flex items-center gap-3">
                      <div className="w-3 h-3 rounded-full bg-blue-500"></div>
                      <div className="text-sm">
                        <span className="text-slate-200 font-medium">Email</span>
                        <p className="text-slate-500 text-xs">Direct communication (From/To/CC) from metadata</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-sm">
                        ...
                        <p className="text-slate-500 text-xs">Appearing in the same document (weighted by frequency)</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-sm">
                        ...
                        <p className="text-slate-500 text-xs">Appearing on the same flight manifest</p>
                      </div>
                    </div>
                  </div>
                </div>
