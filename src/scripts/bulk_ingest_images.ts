...
async function run() {
    ...
    if (!album) {
        ...
        const result = db.prepare('INSERT INTO media_albums (name, description) VALUES (?, ?)').run(albumName, 'Images released on 12.18.25');
...
async function processImage(filePath: string, albumId: number) {
    ...
    const existing = db.prepare('SELECT id FROM media_images WHERE filename = ? AND album_id = ?').get(filename, albumId) as { id: number } | undefined;
    ...
    if (existing) {
        db.prepare(`
            ...
            WHERE id = ?
        `).run(prodPath, thumbPath, width, height, stat.size, dateTaken, cameraMake, cameraModel, orientation, existing.id);
    } else {
        db.prepare(`
        ...
        `).run(
            filename, filename, prodPath, thumbPath, albumId,
            width, height, stat.size, path.extname(filename).slice(1).toLowerCase(), 
            dateTaken, cameraMake, cameraModel, orientation
        );
    }
}
