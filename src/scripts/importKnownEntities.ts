...
function upsertEntity(db: Database.Database, fullName: string, opts?: { primaryRole?: string; connectionsSummary?: string }) {
  const existing = db.prepare('SELECT id FROM entities WHERE full_name = ?').get(fullName) as { id: number } | undefined
  if (existing) {
    if (opts?.primaryRole || opts?.connectionsSummary) {
      db.prepare('UPDATE entities SET primary_role = COALESCE(?, primary_role), connections_summary = COALESCE(?, connections_summary), updated_at = CURRENT_TIMESTAMP WHERE id = ?')
  ...
  const insertTxn = db.transaction(() => {
    names.forEach(n => {
      ...
      if (n.toLowerCase() === 'bubba') {
        ...
        const bill = db.prepare('SELECT id FROM entities WHERE full_name = ?').get('Bill Clinton') as { id: number } | undefined
        if (bill) {
          // Optional: add note to avoid conflation
          db.prepare('UPDATE entities SET connections_summary = COALESCE(connections_summary, ?) WHERE id = ?')
