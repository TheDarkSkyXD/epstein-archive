...
export const InvestigationBoard: React.FC<InvestigationBoardProps> = ({ investigationId }) => {
  ...
  const handleDropOnHypothesis = async (e: React.DragEvent, hypothesisId: string) => {
    ...
    try {
        ...
        const res = await fetch(`/api/investigations/${investigationId}/hypotheses/${hypothesisId}/evidence`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (res.ok) {
            ...
            setHypotheses(prev => prev.map(h => {
                if (h.id === hypothesisId) {
                    return {
                        ...h,
                        evidenceLinks: [...((h as any).evidenceLinks || []), { 
                            id: `temp-${Date.now()}`, 
  ...
  if (loading) return <div className="p-10 text-center text-slate-400">Loading Investigation Board...</div>;
                    ...
                    >
                        ...
                        <div className="mt-3 text-xs text-slate-500 space-y-2">
                             <div className="flex items-center justify-between">
                                 ...
                                 <span className={`px-2 py-0.5 rounded-full ${h.status === 'confirmed' ? 'bg-green-900/50 text-green-400' : 'bg-slate-700'}`}>{h.status}</span>
                             </div>
                                ...
                                <div className="space-y-1 pt-2 border-t border-slate-700/50">
                                    {(h as any).evidenceLinks.map((link: any) => (
                                        <div key={link.id} className="flex items-center gap-1.5 text-slate-400">
                                            <div className={`w-1.5 h-1.5 rounded-full ${link.relevance === 'contradicting' ? 'bg-red-500' : 'bg-green-500'}`} />
                                            <span className="truncate">{link.evidence_title || 'Untitled Evidence'}</span>
                                        </div>
                                    ))}
                                </div>
                             )}
        ...
        <div className="w-1/3 border-r border-slate-800 flex flex-col min-w-[300px] bg-slate-950">
            <div className="p-4 border-b border-slate-800 bg-slate-900/50 flex items-center justify-between">
                ...
                <div className="text-xs text-slate-400">
                    Drag items to Hypotheses or Notebook
                </div>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {evidence.map(e => (
                    ...
                    >
                        ...
                        <div>
                            <h4 className="text-sm font-medium text-slate-200 group-hover:text-blue-300 transition-colors">{e.title}</h4>
                 ...
                 {evidence.length === 0 && (
                    <div className="text-center p-8 text-slate-500 flex flex-col items-center gap-2">
                        ...
                        <p className="text-sm max-w-[200px]">Browse documents or entities and click "Add to Investigation" to collect them here.</p>
                    </div>
                )}
            </div>
        </div>
            ...
            <div className="p-4 border-b border-slate-800 bg-slate-900/50 flex items-center justify-between">
             ...
             >
                <div className="min-h-[200px] p-4 border-2 border-dashed border-slate-800/50 rounded-lg flex flex-col items-center justify-center text-slate-500 hover:bg-slate-800/30 transition-colors">
                    {notebook.length === 0 && <span className="mb-2">Drag evidence here to build your case</span>}
                    ...
                    {/* Notebook Items */}
                     <div className="w-full space-y-2 mt-4">
                        {notebook.map((itemId, idx) => {
                            ...
                            return (
                                <div key={`${itemId}-${idx}`} className="p-3 bg-slate-800 rounded border border-slate-700 flex flex-col gap-2">
