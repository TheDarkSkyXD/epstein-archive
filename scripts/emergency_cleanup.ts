...
const JUNK_TERMS_EXACT = [
  ...
  'Stesday July',
  'Little Saint',
  'Honorable Ruth Milor',
  'Ruth Mier',
  'Ruth Milor', // These look like specific artifacts in this dataset
];
...
function emergencyCleanup() {
  ...
  const epsteinId = db
    .prepare("SELECT id FROM entities WHERE full_name = 'Jeffrey Epstein'")
    .get() as { id: number } | undefined;

  if (epsteinId) {
    const variants = db
      .prepare(`SELECT id, full_name FROM entities WHERE full_name LIKE 'Jeffrey Epstein %'`)
  ...
  const remainingTop = db
    .prepare('SELECT id FROM entities ORDER BY mentions DESC LIMIT 100')
    .all() as { id: number }[];
