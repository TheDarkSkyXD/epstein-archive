...
function assignCovers() {
  ...
  const files = db.prepare("SELECT id, title, metadata_json FROM media_items WHERE title LIKE 'Sascha Barros Testimony%' ORDER BY title ASC").all() as {id: number, title: string, metadata_json: string}[];
