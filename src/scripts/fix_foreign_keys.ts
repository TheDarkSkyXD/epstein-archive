...
try {
  ...
  db.transaction(() => {
    ...
    db.prepare('CREATE INDEX IF NOT EXISTS idx_entity_mentions_context ON entity_mentions(context_type)').run();
    ...
    db.prepare('CREATE INDEX IF NOT EXISTS idx_timeline_events_date ON timeline_events(event_date DESC)').run();
    db.prepare('CREATE INDEX IF NOT EXISTS idx_timeline_events_type ON timeline_events(event_type)').run();
