...
function fixSaschaTranscripts() {
  ...
  const files = db
    .prepare(
      "SELECT id, title, metadata_json FROM media_items WHERE title LIKE 'Sascha Barron Testimony - Part%'",
    )
    ...
    for (const record of files) {
      ...
      if (fs.existsSync(txtPath)) {
        ...
        meta.chapters = [
          { title: 'Start', start: 0 },
          ...segments
            .filter((s, i) => i > 0 && i % 20 === 0)
            .map((s) => ({ title: `Segment ${Math.floor(s.start / 60)}m`, start: s.start })),
