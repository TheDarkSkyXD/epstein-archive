...
async function mergeEpsteinEntities() {
  ...
  variants.forEach((v: any) => {
    console.log(`  - ${v.full_name} (ID: ${v.id}, Mentions: ${v.mentions}, Spice: ${v.spice_rating})`);
  });
  ...
  for (const duplicate of duplicates) {
    ...
    const mentionsUpdated = db.prepare(`
    ...
    const evidenceUpdated = db.prepare(`
    ...
    const timelineUpdated = db.prepare(`
