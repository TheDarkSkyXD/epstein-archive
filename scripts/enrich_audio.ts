...
const ALBUMS = [
  {
    ...
    description: 'Interviews and testimony from Manuel Sascha Barros, detailing experiences and providing forensic context.',
...
function getOrCreateAlbum(name: string, description: string) {
  const row = db.prepare('SELECT id FROM media_albums WHERE name = ?').get(name) as { id: number } | undefined;
  if (row) return row.id;

  const result = db.prepare('INSERT INTO media_albums (name, description) VALUES (?, ?)').run(name, description);
  return result.lastInsertRowid as number;
}

function processAudioFiles() {
  const audioFiles = db.prepare<[], { id: number; file_path: string; description: string; title: string }>(
