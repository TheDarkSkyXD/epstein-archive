...
function run() {
  try {
    ...
    if (unknownCount.count > 0) {
      ...
      db.prepare(`UPDATE entities SET type = 'Person' WHERE name LIKE '%Jeffrey Epstein%' OR name LIKE '%Donald Trump%' OR name LIKE '%Virginia Giuffre%' OR name LIKE '%Virginia Roberts%'`).run();
      db.prepare(`UPDATE entities SET type = 'Location' WHERE name LIKE '%Palm Beach%' OR name LIKE '%New York%' OR name LIKE '%Manhattan%'`).run();
    ...
    if (isolates.length > 0) {
        ...
        const epstein = db.prepare("SELECT id FROM entities WHERE name LIKE '%Jeffrey Epstein%' LIMIT 1").get() as { id: number };
