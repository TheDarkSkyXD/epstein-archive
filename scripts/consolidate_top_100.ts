...
function consolidateTopEntities() {
  ...
  const allPersons = db
    .prepare("SELECT id, full_name FROM entities WHERE entity_type = 'Person'")
  ...
  db.transaction(() => {
    for (const target of topEntities) {
      ...
      if (
        ['The', 'Of', 'In', 'To', 'And'].some(
          (w) => targetNameRaw.includes(w) && targetNameRaw.length < 8,
        )
      )
        continue;
      ...
      if (candidates.length > 0) {
        ...
        for (const cand of candidates) {
          ...
          console.log(
            `   Merging duplicate: ${cand.full_name} (${cand.id}) -> ${target.full_name}`,
          );
