...
function enrichSaschaAudio() {
  ...
  const albumDesc = "Full testimony audio files. Warning: Contains explicit descriptions of abuse. Credits: Sascha Barron and Lisa Noelle Voldeng.";
  
  let album = db.prepare('SELECT id FROM media_albums WHERE name = ?').get(albumName) as {id: number} | undefined;
  
  if (!album) {
      console.log('   Creating Album...');
      const info = db.prepare('INSERT INTO media_albums (name, description, is_sensitive) VALUES (?, ?, 1)').run(albumName, albumDesc);
      album = { id: info.lastInsertRowid as number };
  } else {
      console.log('   Updating Album metadata...');
      db.prepare('UPDATE media_albums SET description = ?, is_sensitive = 1 WHERE id = ?').run(albumDesc, album.id);
  }
  
  // 2. Find Files and Update
  // Files are named SRTestimonyA... SRTestimonyF...
  const files = db.prepare("SELECT id, file_path, metadata_json FROM media_items WHERE file_path LIKE '%SRTestimony%'").all() as {id: number, file_path: string, metadata_json: string}[];
      ...
      for (const file of files) {
          ...
          try {
              ...
              if (!meta.transcript_segments || meta.transcript_segments.length === 0) {
                   console.log(`   ⚠️ WARNING: No transcript segments found for ${newTitle}. Please re-run ingest_audio.ts if needed.`);
              } else {
              ...
              if (!meta.chapters || meta.chapters.length === 0) {
                  ...
                  // Update JSON
                  db.prepare('UPDATE media_items SET metadata_json = ? WHERE id = ?').run(JSON.stringify(meta), file.id);
              }
