...
export const DocumentBrowser: React.FC<DocumentBrowserProps> = ({
  ...
  const DocumentCard: React.FC<{ document: Document }> = ({ document }) => {
    ...
    return (
      <div
        className="bg-gray-900 border border-gray-700 rounded-lg p-4 hover:border-gray-500 transition-colors cursor-pointer"
      ...
      >
        ...
        <h3 className="font-semibold text-white mb-1 line-clamp-2">
          {effectiveSearchTerm
            ? renderHighlightedText(displayTitle, effectiveSearchTerm)
            : displayTitle}
        </h3>
        ...
        {summary && (
          <p className="text-sm text-gray-300 mb-3 line-clamp-3">
            {effectiveSearchTerm
              ? renderHighlightedText(summary + '...', effectiveSearchTerm)
              : summary + '...'}
          </p>
        )}
          ...
          <div className="mt-3 flex flex-wrap gap-1">
            {(document.entities || []).slice(0, 3).map((entity, index) => (
              <span key={index} className="px-2 py-1 bg-gray-800 text-xs text-gray-300 rounded">
                {effectiveSearchTerm
                  ? renderHighlightedText(entity.name, effectiveSearchTerm)
                  : entity.name}
              </span>
            ))}
