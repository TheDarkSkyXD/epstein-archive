...
export const AIRPORT_COORDS: Record<string, { lat: number; lng: number; city: string }> = {
  ...
  'KLAS': { lat: 36.0840, lng: -115.1537, city: 'Las Vegas, NV' },
...
export const flightsRepository = {
  getFlights: (filters: {
    ...
    if (passenger) {
      conditions.push('f.id IN (SELECT flight_id FROM flight_passengers WHERE passenger_name LIKE ?)');
  ...
  getFlightById: (id: number): Flight | null => {
    ...
    flight.passengers = db.prepare('SELECT * FROM flight_passengers WHERE flight_id = ?').all(id) as FlightPassenger[];
  ...
  getFlightStats: (): FlightStats => {
    ...
    const uniquePassengers = (db.prepare(
      'SELECT COUNT(DISTINCT passenger_name) as count FROM flight_passengers'
    ...
    const flightsByYear = db.prepare(`
